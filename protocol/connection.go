...
func Accept(c net.Conn, host string, port int) (*Conn, error) {
	return &Conn{
		r:                    c,
		w:                    c,
		net:                  c,
...
func (c *Conn) WriteRaw(buf []byte) error {
	c.net.SetWriteDeadline(time.Now().Add(15 * time.Second))
	WriteVarInt(c.w, VarInt(len(buf)))
	_, err := c.w.Write(buf)
	return err
}

func (c *Conn) WriteBuffer(buf *bytes.Buffer) error {
	uncompessedSize := 0
	extra := 0
...
func (c *Conn) readPacket() (Packet, error) {
	...
	if err := packet.read(r); err != nil {
		return packet, fmt.Errorf("packet(%s:0x%02X)/%s: %s", c.State, id, reflect.TypeOf(packet), err)
	}
	...
	if r.Len() > 0 {
		return packet, fmt.Errorf("Didn't finish reading packet %s:0x%02X, have %d bytes left: %v", c.State, id, r.Len(), buf)
	}
	if c.Logger != nil {
