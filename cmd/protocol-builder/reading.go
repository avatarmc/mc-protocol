...
func (r *reading) readType(e ast.Expr, name string, tag reflect.StructTag) {
	...
	case *ast.ArrayType:
		...
		elTypeName := fmt.Sprint(e.Elt)

		if star, ok := e.Elt.(*ast.StarExpr); ok {
			left := star.X.(*ast.SelectorExpr).X
			right := star.X.(*ast.SelectorExpr).Sel

			switch fmt.Sprint(left) {
			case "nbt":
				imports[ImportPrefix+"/encoding/nbt"] = struct{}{}
			}

			elTypeName = fmt.Sprintf("*%s.%s", left, right)
		}

		fmt.Fprintf(&r.buf, "%s = make([]%s, %s)\n", name, elTypeName, lenVar)
...
func (r *reading) readNamed(t, name string, tag reflect.StructTag) {
	...
	if len(funcName) != 0 {
		if notProtocol {
			...
			imports[ImportPrefix+"/protocol"] = struct{}{}
		}
